...
def get_pesos_por_area(df_pesos_actuales: pd.DataFrame) -> dict:
    equipos_area = {
        ...
        'C': ['Equipo_C1', 'Equipo_C2', 'Equipo_C3'],
...
# Lee exactamente la celda BK5 de la hoja TTAA de OBS-DISTRI ATENEA
...
if pd.isna(cutoff_dt):
    raise ValueError("Cutoff inválido en TTAA!BK5 (esperaba fecha/hora).")
...
num_C = df_cad.iloc[4:7]['CUPONES_PRELIM_A'].sum()
den_C = df_cad.iloc[4:7]['HORAS'].sum() / 6
...
# %%
equipos_C = ["Equipo_C1", "Equipo_C2","Equipo_C3"]
...
def distribuir_area_T(
    ...
    equipos_T = ['Equipo_A1', 'Equipo_A2', 'Equipo_B1', 'Equipo_B2', 'Equipo_C1', 'Equipo_C2', 'Equipo_C3']

    # Map directoras
    IG_teams = {'Equipo_A1', 'Equipo_B1', 'Equipo_C1', 'Equipo_C3'}
...
def distribuir_area_E(df_fresh, df_hist_total, df_pesos_areas, df_reap_validas):
    """
    Área E:
    1) Sin cadencia. Objetivo 1: IG=54% vs XP=46% del total (hist + reaps + fresh).
    ...
    if len(IG_equipos) == 0 or len(XP_equipos) == 0:
        print("ADVERTENCIA: Falta al menos un grupo de directora en los equipos de E. "
              "Verifica que E incluya A1,B1,C1,C3 (IG) y A2,B2,C2 (XP).")
    ...
    target_IG_total = int(round(0.54 * total_E))
    ...
    df_resumen_directora = pd.DataFrame({
        ...
        'Share_Target': [0.54, 0.46],
        'Delta_puntos': [share_IG - 0.54, share_XP - 0.46]
    }, index=['IG', 'XP'])
    ...
    if (final_IG != target_IG_total) or (final_XP != target_XP_total):
        msgs.append("Nota: No se pudo igualar exactamente 54/46 por fijos históricos/reaperturas. "
...
df_final_total.to_excel("Distribucion_Final_I.xlsx", index=False)
...
df_final_ajustado.to_excel("Distribucion_Final_Ajustada.xlsx", index=False)
...
## Desde la carpeta donde está distribucion_OBS.py para compilar: (quitar los "#")
...
#  --collect-submodules country_converter `
#  --add-binary "C:\Users\uscp9a\AppData\Local\Programs\Python\Python313\Lib\site-packages\pulp\solverdir\cbc\win\i64\cbc.exe;pulp/solverdir/cbc/win/i64" `
#  ".\distribucion_OBS.py"
